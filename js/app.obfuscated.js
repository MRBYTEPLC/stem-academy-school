...
fn_showTimeoutWarning() {
  ...
  d0c.getElementById('timeout-count').textContent = warnCountdown;
  d0c.getElementById('timeout-overlay').classList.add('show');
...
fn_hideTimeoutOverlay() {
  d0c.getElementById('timeout-overlay').classList.remove('show');
  clearInterval(warnInterval);
}
...
fn_logOut() {
  ...
  ['home','app'].forEach(id => d0c.getElementById(id).classList.remove('active'));
  d0c.getElementById('login').classList.add('active');
  // Clear input fields for next user
  d0c.getElementById('login-user').value = '';
  d0c.getElementById('login-pass').value = '';
  d0c.getElementById('login-error').textContent = '';
  // Restore card animation
  var card = d0c.querySelector('.login-card');
...
['mousemove','mousedown','keydown','touchstart','touchmove','scroll','wheel','click']
  .forEach(evt => d0c.addEventListener(evt, resetIdleTimer, { passive: true }));

// ── Login ───────────────────────────────────────────────────────────────────
fn_doLogin() {
  var user = d0c.getElementById('login-user').value.trim().toLowerCase();
  var pass = d0c.getElementById('login-pass').value;
...
d0c.addEventListener('keydown', e => {
  if (e.key === 'Enter' && d0c.getElementById('login').classList.contains('active')) {
    doLogin();
  }
});
...
fn_initCounters() {
  ...
  fetch(`https://api.countapi.xyz/hit/${COUNT_NS}/${COUNT_KEY}`)
    ...
    .then(data => {
      var v = data.value || 0;
      d0c.getElementById('visit-count').textContent = v.toLocaleString();
    })
    .catch(() => {
      ...
      d0c.getElementById('visit-count').textContent = v;
    });

  // ── Likes — per-device (no backend needed for likes) ───────────
  d0c.getElementById('like-count').textContent =
    localStorage.getItem('sa_likes') || '0';

  // ── Online now — realistic random (no real-time backend) ────────
  d0c.getElementById('online-count').textContent =
...
fn_addLike() {
  ...
  d0c.getElementById('like-count').textContent = likes;
  showToast('\u2764\uFE0F  Thanks for the like!  Total: ' + likes);
  var btn = d0c.querySelector('.like-btn');
...
fn_goHome() {
  d0c.getElementById('home').classList.add('active');
  d0c.getElementById('app').classList.remove('active');
  resetIdleTimer();
}

fn_openApp(idx) {
  d0c.getElementById('home').classList.remove('active');
  d0c.getElementById('app').classList.add('active');
  switchMod(idx);
  resizeMC();
}
...
fn_switchMod(idx) {
  ...
  d0c.querySelectorAll('.tab-btn').forEach((b, i) => b.classList.toggle('active', i === idx));

  // Modules 0 and 1 use the main canvas; 2-5 use HTML sim panels
  var useCanvas = idx === 0 || idx === 1;
  d0c.getElementById('cv-wrap').style.display = useCanvas ? 'block' : 'none';
  [2, 3, 4, 5].forEach(n => {
    var p = d0c.getElementById('sim' + n);
    if (p) p.classList.toggle('active', n === idx);
  });

  // Fill sidebar with instructions for this module
  d0c.getElementById('sidebar').innerHTML = SIDEBARS[idx] || '';
  ...
  d0c.getElementById('run-btn').style.opacity   = useCanvas ? '1' : '0.4';
  d0c.getElementById('erase-btn').textContent   = useCanvas ? '🗑 ERASE' : '↺ RESET';
}

fn_handleRun()   { if (curMod === 0 || curMod === 1) evalNet(); }
fn_handleErase() {
...
var SIDEBARS = {
  ...
  <div class="sb-sec"><h3 class="sb-h-p">⚡ LOGIC GATES — INSTRUCTIONS</h3>
    ...
    <div class="icard ig"><strong>⚙️ OR Gate</strong>TRUE if A OR B (or both) is ON.<div class="code-ex">Alarm: rings if front door OR w0w opens.</div></div>
...
w0w.addEventListener('resize', resizeMC);
...
fn_resetMC() { nodes={}; conns=[]; nc=0; selPin=null; dragId=null; drawMC(); }
...
fn_dist2(a,b) { return (a.x-b.x)**2+(a.y-b.y)**2; }
fn_inNode(p,n)  { return p.x>=n.x&&p.x<=n.x+n.w&&p.y>=n.y&&p.y<=n.y+n.h; }
...
fn_openEdit(nid) {
  editNodeId = nid;
  d0c.getElementById('edit-val').value = nodes[nid].val;
  d0c.getElementById('edit-overlay').classList.add('show');
  d0c.getElementById('edit-val').focus();
  d0c.getElementById('edit-val').select();
}
fn_confirmEdit() {
  if (editNodeId && nodes[editNodeId]) {
    var v = parseFloat(d0c.getElementById('edit-val').value);
    if (!isNaN(v)) nodes[editNodeId].val = v;
  }
  cancelEdit(); drawMC();
}
fn_cancelEdit() { editNodeId=null; d0c.getElementById('edit-overlay').classList.remove('show'); }
// Close edit with Enter key
d0c.getElementById('edit-val').addEventListener('keydown', e => { if(e.key==='Enter') confirmEdit(); if(e.key==='Escape') cancelEdit(); });
...
fn_showCF(tab) {
  ['if','for','while','dw'].forEach(t => {
    d0c.getElementById('cf-'+t).style.display = t===tab ? 'block' : 'none';
    var btn = d0c.getElementById('cf-t-'+t);
    if (btn) btn.className = t===tab ? 'gbtn go' : 'gbtn purp';
  });
}

fn_ifVarChange() {
  var isDay = d0c.getElementById('if-var').value === 'day';
  d0c.getElementById('if-num-row').style.display = isDay ? 'none' : 'flex';
  d0c.getElementById('if-day-row').style.display = isDay ? 'flex' : 'none';
}

fn_traceOut(id, lines) {
  var el = d0c.getElementById(id);
  el.textContent = lines.join('\n');
  el.scrollTop = el.scrollHeight;
}

fn_runIF() {
  var v=d0c.getElementById('if-var').value, lines=[];
  if (v==='day') {
    var day=d0c.getElementById('if-day').value;
    ...
    var val=+d0c.getElementById('if-val').value, thr=+d0c.getElementById('if-thresh').value;
...
fn_runFOR() {
  var s=+d0c.getElementById('for-s').value;
  var e=+d0c.getElementById('for-e').value;
  var st=Math.max(1,+d0c.getElementById('for-st').value);
  var actKey=d0c.getElementById('for-act').value;
...
fn_runWHILE() {
  var s=+d0c.getElementById('wh-s').value;
  var lim=+d0c.getElementById('wh-l').value;
  var inc=Math.max(1,+d0c.getElementById('wh-i').value);
  var ctx=d0c.getElementById('wh-ctx').value;
...
fn_runDW() {
  var s=+d0c.getElementById('dw-s').value;
  var lim=+d0c.getElementById('dw-l').value;
  var ctx=d0c.getElementById('dw-ctx').value;
...
/* ================================================================
   ...
   ================================================================ */
var plcc   = d0c.getElementById('plc-canvas');
...
fn_initPLC() {
  ...
  drawPLC(); d0c.getElementById('plc-status').textContent='Status: Add blocks and wire them...';
}
...
fn_plcUp(){pDragId=null;}
fn_plcPinClick(tag,nid){
...
fn_runPLC(){
  var target=parseInt(d0c.getElementById('plc-tgt').value)||8;
  ...
  pAnimId=setInterval(()=>{
    ...
    if(ctrN.count>=ctrN.target){
      ...
      d0c.getElementById('plc-status').textContent=`✅ DONE! ${ctrN.count}/${ctrN.target} boxes — MOTOR STOPPED AUTO`;
      showToast(`🎉 PLC Cycle Complete!\n${ctrN.count} boxes counted → Motor stopped automatically!\nThis is real PLC ladder logic!`);
    } else {
      d0c.getElementById('plc-status').textContent=`⚙️ Running — ${ctrN.count}/${ctrN.target} boxes | Motor: ${motorN.running?'ON':'OFF'}`;
    }
    drawPLC();
  },65);
}
fn_emergencyStop(){
  ...
  d0c.getElementById('plc-status').textContent='🛑 E-STOP ACTIVE — All outputs CLEARED immediately!';
...
fn_resetPLC(){
  ...
  d0c.getElementById('plc-status').textContent='Status: Add blocks and wire them...';
  drawPLC();
}
...
fn_resetSorter(){
  ...
  d0c.getElementById('s-lc').textContent='0';
  d0c.getElementById('s-rc').textContent='0';
  d0c.getElementById('s-proc').textContent='0 / 6';
  drawSortIdle();
}

fn_drawSortIdle(hl){
  var cv=d0c.getElementById('sort-cv'); if(!cv)return;
...
fn_runSorter(){
  var ifc=d0c.getElementById('s-ifc').value;
  var ifb=d0c.getElementById('s-ifb').value;
  var elb=d0c.getElementById('s-elb').value;
...
fn_animSort(){
  ...
  sIdx++;
  d0c.getElementById('s-lc').textContent=sL;
  d0c.getElementById('s-rc').textContent=sR;
  d0c.getElementById('s-proc').textContent=`${sIdx} / ${SORT_SEQ.length}`;
  var cv=d0c.getElementById('sort-cv'); if(!cv)return;
  cv.width=cv.offsetWidth||600; cv.height=155;
  var cx=cv.getContext('2d'),W=cv.width;
  drawSortIdle(dest);
...
fn_initEnergy() { ELINES.forEach(l=>l.on=false); eSolves=0; renderECards(); updateEMetrics(); }

fn_renderECards(){
  var el=d0c.getElementById('e-cards'); if(!el)return;
  el.innerHTML='';
  ELINES.forEach(l=>{
    var d=d0c.createElement('div');
...
fn_updateEMetrics(){
  ...
  var pb=d0c.getElementById('e-pb'),pv=d0c.getElementById('e-pv');
  var gb=d0c.getElementById('e-gb'),gv=d0c.getElementById('e-gv');
  ...
  var es=d0c.getElementById('e-status');
...
fn_commitEnergy(){
  ...
  if(pOk&&gOk){
    eSolves++;
    d0c.getElementById('e-score').textContent=`🏆 Valid solutions found this session: ${eSolves}`;
...
var toastTimer=null;
fn_showToast(msg){
  var t=d0c.getElementById('toast');
...
/* ================================================================
   ...
   ================================================================ */
w0w.addEventListener('DOMContentLoaded',()=>{
  d0c.getElementById('sidebar').innerHTML=SIDEBARS[0];
  resizeMC();
  showCF('if');
  // Size the PLC canvas
  var pc=d0c.getElementById('plc-canvas');
  if(pc){pc.width=pc.offsetWidth||600;pc.height=280;}
});
w0w.addEventListener('resize',()=>{
  resizeMC();
  var pc=d0c.getElementById('plc-canvas');
