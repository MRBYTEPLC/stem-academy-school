...
let idleTimer = null;
let warnTimer = null;
let warnCountdown = WARN_SECONDS;
...
const COUNT_NS = 'stem-code-academy-v4';
const COUNT_KEY = 'logins';
...
function initCounters() {
 ...
 let v = parseInt(localStorage.getItem('sa_visits') || '0') + 1;
...
function addLike() {
 let likes = parseInt(localStorage.getItem('sa_likes') || '0') + 1;
...
let nodes = {}, conns = [], nc = 0;
let selPin = null, dragId = null, dragOX = 0, dragOY = 0, lastClick = 0;
let editNodeId = null;
const C = {
...
function spawnGate(type) {
 ...
 nc++; const id = `n${nc}`;
 const h = type==='IF_ELSE'?118 : ['FOR','WHILE','DO_WHILE'].includes(type)?100 : 92;
...
function drawMC() {
 ...
 for (let i=0; i<mc.width; i+=40) { ctx.beginPath(); ctx.moveTo(i,0); ctx.lineTo(i,mc.height); ctx.stroke(); }
 for (let j=0; j<mc.height; j+=40) { ctx.beginPath(); ctx.moveTo(0,j); ctx.lineTo(mc.width,j); ctx.stroke(); }
 
 conns.forEach(c => {
 const n1 = nodes[c.from], n2 = nodes[c.to]; if (!n1 || !n2) return;
 let ox = n1.x + n1.w, oy = n1.y + n1.h/2;
 if (n1.type==='IF_ELSE') oy = c.oIdx===0 ? n1.y+60 : n1.y+92;
 let ix = n2.x;
 let iy = ['AND','OR','NAND','OP'].includes(n2.type)
 ? (c.iIdx===0 ? n2.y+26 : n2.y+n2.h-26)
 : n2.y + n2.h/2;
 if (n2.type==='TERM') iy = n2.y + n2.h/2;
 ctx.beginPath(); ctx.moveTo(ox, oy);
 ctx.bezierCurveTo((ox+ix)/2, oy, (ox+ix)/2, iy, ix, iy);
 const sv = n1.type==='SW' ? n1.val : n1.type==='NUM' ? n1.val : n1.outVal;
 const active = sv===true || sv==='TRUE' || typeof sv==='number' ||
 ...
 const ol = {ADD:'➕ ADD',MUL:'✖ MULTIPLY',SUB:'➖ SUBTRACT',DIV:'➗ DIVIDE',CMP:'⚖ COMPARE'};
 ...
 const good = n.disp==='TRUE'||Number(n.disp)>0||
...
function getMPos(e) {
 ...
 const sx = mc.width/r.width, sy = mc.height/r.height;
 const cx = e.touches ? e.touches[0].clientX : e.clientX;
 const cy = e.touches ? e.touches[0].clientY : e.clientY;
 return { x:(cx-r.left)*sx, y:(cy-r.top)*sy };
}
function mcDown(e) {
 const p = getMPos(e), now = Date.now();
...
function mcMove(e) {
 if (!dragId) return;
 const p = getMPos(e), n = nodes[dragId];
...
function pinClick(tag) {
 ...
 if (tag.includes('_in')) {
 const fp=selPin.split('_out'), fn=fp[0], oi=fp[1]===''?0:parseInt(fp[1]);
 const tp=tag.split('_in'), tn=tp[0], ii=parseInt(tp[1]);
...
function confirmEdit() {
 ...
 const v = parseFloat(document.getElementById('edit-val').value);
...
function evalNet() {
 ...
 let changed=true, it=0, max=Object.keys(nodes).length*6;
 while (changed && it<max) {
 changed=false; it++;
 Object.values(nodes).forEach(n => {
 
 if (n.type==='OP' && n.outVal===null) {
 const a=mathUp(n.id,0), b=mathUp(n.id,1);
 if (a!==null && b!==null) {
 ...
 if (n.type==='TERM' && n.disp==='---') {
 const v=mathUp(n.id,0);
 if (v!==null) { n.disp=String(v); changed=true; return; }
 const v2=logicUp(n.id,0);
 if (v2!==null) { n.disp=String(v2).toUpperCase(); changed=true; }
 }
 
 if (n.type==='NOT' && n.outVal==='?') { const i0=logicUp(n.id,0); if(i0!==null){n.outVal=!i0;changed=true;} }
 if (['AND','OR','NAND'].includes(n.type) && n.outVal==='?') {
 const i0=logicUp(n.id,0), i1=logicUp(n.id,1);
 if (i0!==null && i1!==null) {
 n.outVal = n.type==='AND'?(i0&&i1):n.type==='OR'?(i0||i1):!(i0&&i1); changed=true;
 }
 }
 if (n.type==='IF_ELSE' && n.outVal==='?') { const c=logicUp(n.id,0); if(c!==null){n.outVal=c;changed=true;} }
 if (['FOR','WHILE','DO_WHILE'].includes(n.type) && n.outVal==='?') {
 const c=logicUp(n.id,0);
 if (c!==null) {
...
function mathUp(tid,idx) {
 const c=conns.find(c=>c.to===tid&&c.iIdx===idx); if(!c)return null;
 const s=nodes[c.from]; if(!s)return null;
...
function showCF(tab) {
 ...
 const btn = document.getElementById('cf-t-'+t);
 if (btn) btn.className = t===tab ? 'gbtn go' : 'gbtn purp';
 });
}
function ifVarChange() {
 const isDay = document.getElementById('if-var').value === 'day';
...
function runIF() {
 const v=document.getElementById('if-var').value, lines=[];
 if (v==='day') {
 const day=document.getElementById('if-day').value;
 ...
 const val=+document.getElementById('if-val').value, thr=+document.getElementById('if-thresh').value;
...
function runFOR() {
 const s=+document.getElementById('for-s').value;
 const e=+document.getElementById('for-e').value;
 const st=Math.max(1,+document.getElementById('for-st').value);
 const actKey=document.getElementById('for-act').value;
 const acts={cookie:'🍪 Drop cookie into box',stamp:'🏷️ Stamp product label',weld:'🔧 Weld joint segment',alarm:'🚨 Sound alarm beep',bell:'🔔 Ring school bell',print:'🖨️ Print a line'};
 const act=acts[actKey]||actKey;
 const lines=['# FOR Loop execution',''];
...
function runWHILE() {
 const s=+document.getElementById('wh-s').value;
 const lim=+document.getElementById('wh-l').value;
 const inc=Math.max(1,+document.getElementById('wh-i').value);
 const ctx=document.getElementById('wh-ctx').value;
 const acts={pump:'🏭 Run drain pump cycle',fan:'💨 Keep cooling fan ON',snooze:'😴 Hit snooze button',count:'🔢 Increment counter'};
 const act=acts[ctx]||'Execute action';
 const lines=['# WHILE Loop execution',''];
...
function runDW() {
 const s=+document.getElementById('dw-s').value;
 const lim=+document.getElementById('dw-l').value;
 const ctx=document.getElementById('dw-ctx').value;
 const acts={alarm:'🚨 Sound alarm siren',test:'🔧 Run machine self-test',hw:'📚 Check homework',menu:'🖥️ Show application menu'};
 const act=acts[ctx]||'Execute action';
 const lines=['# DO-WHILE Loop execution',''];
...
const plcc = document.getElementById('plc-canvas');
const plcCtx = plcc ? plcc.getContext('2d') : null;
let pNodes={},pConns=[],pNc=0,pSelPin=null,pDragId=null,pDOX=0,pDOY=0,pLastClick=0;
let pRunning=false,pAnimId=null,pBoxes=[];
function initPLC() {
...
function getPLCPins(n) {
 ...
 ps.push({x:n.x, y:n.y+24, tag:`${n.id}_in0`}); 
 ps.push({x:n.x+n.w, y:n.y+24, tag:`${n.id}_out`}); 
 ps.push({x:n.x+n.w, y:n.y+n.h-18, tag:`${n.id}_done`}); 
 } else if(n.type==='MOTOR'){
 ps.push({x:n.x,y:n.y+18, tag:`${n.id}_in0`}); 
 ps.push({x:n.x,y:n.y+n.h-18,tag:`${n.id}_stop`}); 
...
function drawPLC() {
 if(!plcCtx)return;
 const W=plcc.width||600, H=plcc.height||280;
 plcCtx.clearRect(0,0,W,H);
 
 plcCtx.strokeStyle='#060d1a'; plcCtx.lineWidth=1;
 for(let i=0;i<W;i+=30){plcCtx.beginPath();plcCtx.moveTo(i,0);plcCtx.lineTo(i,H);plcCtx.stroke();}
 for(let j=0;j<H;j+=30){plcCtx.beginPath();plcCtx.moveTo(0,j);plcCtx.lineTo(W,j);plcCtx.stroke();}
 ...
 for(let i=0;i<W;i+=30){plcCtx.strokeStyle='#222';plcCtx.lineWidth=1;plcCtx.beginPath();plcCtx.moveTo(i,H-40);plcCtx.lineTo(i,H-22);plcCtx.stroke();}
 plcCtx.fillStyle='#444'; plcCtx.fillText && plcCtx.fillText;
 
 pConns.forEach(c=>{
 const n1=pNodes[c.fn],n2=pNodes[c.tn]; if(!n1||!n2)return;
 const op=getPLCPins(n1).find(p=>p.tag===c.ft);
 const ip=getPLCPins(n2).find(p=>p.tag===c.tt);
 if(!op||!ip)return;
 plcCtx.beginPath();plcCtx.moveTo(op.x,op.y);
 plcCtx.bezierCurveTo((op.x+ip.x)/2,op.y,(op.x+ip.x)/2,ip.y,ip.x,ip.y);
 
 let active=false;
 const src=pNodes[c.fn];
...
function plcGetPos(e) {
 const r=plcc.getBoundingClientRect();
 const cx=e.touches?e.touches[0].clientX:e.clientX;
 const cy=e.touches?e.touches[0].clientY:e.clientY;
...
function plcDown(e){
 if(!plcc)return;
 const p=plcGetPos(e),now=Date.now();
 if(now-pLastClick<320){
 for(const n of Object.values(pNodes)){
 ...
 if(n.type==='COUNTER'){const v=prompt('Set COUNTER target:',n.target);if(v&&!isNaN(+v))n.target=+v;drawPLC();return;}
 }
 }
 }
 pLastClick=now;
 for(const n of Object.values(pNodes)){
 for(const pin of getPLCPins(n)){
...
function plcMove(e){
 if(!pDragId)return;
 const p=plcGetPos(e),n=pNodes[pDragId];
...
function runPLC(){
 const target=parseInt(document.getElementById('plc-tgt').value)||8;
 Object.values(pNodes).forEach(n=>{if(n.type==='COUNTER'){n.target=target;n.count=0;}});
 const motor=Object.values(pNodes).find(n=>n.type==='MOTOR');
 const start=Object.values(pNodes).find(n=>n.type==='START');
 const counter=Object.values(pNodes).find(n=>n.type==='COUNTER');
 ...
 pRunning=true; pBoxes=[]; let tick=0;
 if(pAnimId) clearInterval(pAnimId);
 const W=plcc.width||600, H=plcc.height||280;
 const sensorN=Object.values(pNodes).find(n=>n.type==='SENSOR');
 pAnimId=setInterval(()=>{
 tick++;
 const estopN=Object.values(pNodes).find(n=>n.type==='ESTOP');
 const startN=Object.values(pNodes).find(n=>n.type==='START');
 const motorN=Object.values(pNodes).find(n=>n.type==='MOTOR');
 const ctrN=Object.values(pNodes).find(n=>n.type==='COUNTER');
 const senN=Object.values(pNodes).find(n=>n.type==='SENSOR');
...
const SORT_SEQ = ['Red','Green','Red','Green','Green','Red'];
let sIdx=0, sL=0, sR=0, sAnimId=null;
...
function drawSortIdle(hl){
 const cv=document.getElementById('sort-cv'); if(!cv)return;
 cv.width=cv.offsetWidth||600; cv.height=155;
 const cx=cv.getContext('2d'), W=cv.width;
...
function runSorter(){
 const ifc=document.getElementById('s-ifc').value;
 const ifb=document.getElementById('s-ifb').value;
 const elb=document.getElementById('s-elb').value;
 ...
 const redDest=ifc==='Red'?ifb:elb;
 const grnDest=ifc==='Green'?ifb:elb;
...
function animSort(){
 ...
 const color=SORT_SEQ[sIdx];
 const dest=color==='Red'?'Left':'Right';
...
function renderECards(){
 const el=document.getElementById('e-cards'); if(!el)return;
 el.innerHTML='';
 ELINES.forEach(l=>{
...
function toggleLine(lid){
 const l=ELINES.find(e=>e.id===lid); if(l)l.on=!l.on;
 renderECards(); updateEMetrics();
}
function updateEMetrics(){
 let tp=0,tg=0;
 ELINES.forEach(l=>{if(l.on){tp+=l.prod;tg+=l.pwr;}});
 const pOk=tp>=100, gOk=tg<=120;
 const pb=document.getElementById('e-pb'),pv=document.getElementById('e-pv');
 const gb=document.getElementById('e-gb'),gv=document.getElementById('e-gv');
...
function commitEnergy(){
 let tp=0,tg=0;
 const active=ELINES.filter(l=>l.on);
...
window.addEventListener('DOMContentLoaded',()=>{
 ...
 const pc=document.getElementById('plc-canvas');
