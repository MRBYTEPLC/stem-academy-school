...
var SIDEBARS = {
  ...
  6: `<div class="sb-sec"><h3 class="sb-h-c">🤖 CODE FLOW 2 — QUICK REF</h3>
    ...
    <div class="icard ic"><strong>WHILE</strong>Checks BEFORE each run. Loops while condition stays TRUE.<div class="code-ex">WHILE still_sleeping: RingAlarm</div></div>
    ...
    <div class="icard ic"><strong>FOR</strong>Repeats an exact number of times.<div class="code-ex">FOR slot 1 TO 6: PlaceEgg(slot)</div></div>
...
/* ================================================================
   [SECTION: CODE FLOW 2 — extra real-life examples]
   IF/ELSEIF/ELSE, WHILE, Nested IF, FOR, AND Logic, TEST/CASE
   ================================================================ */
var CF2_TABS = ['traffic','alarm','atm','eggs','ac','robot'];

function showCF2(tab) {
  CF2_TABS.forEach(t => {
    var pane = document.getElementById('cf2-'+t);
    if (pane) pane.style.display = (t===tab) ? 'block' : 'none';
    var btn = document.getElementById('cf2-t-'+t);
    if (btn) btn.className = (t===tab) ? 'gbtn go' : 'gbtn purp';
  });
}
...
function runCF2Alarm() {
  var snoozes = Math.max(0, +document.getElementById('cf2-snoozes').value || 0);
  var lines = ['# The Stubborn Alarm Clock — WHILE Loop', ''];
  lines.push('still_sleeping = TRUE', '');
  for (var i = 1; i <= snoozes + 1; i++) {
    lines.push(`WHILE still_sleeping == TRUE?  →  TRUE ✅ (run #${i})`);
    lines.push('  → RingAlarm 🔔');
    lines.push('  → Wait5Minutes ⏳');
    if (i <= snoozes) {
      lines.push('  → (you hit snooze... still_sleeping stays TRUE)', '');
    } else {
      lines.push('  → still_sleeping := FALSE   (you got up! 🦵)', '');
    }
  }
  lines.push('WHILE still_sleeping == TRUE?  →  FALSE ❌ → EXIT loop');
  lines.push('', `⬆ The alarm rang ${snoozes + 1} time(s) total — it only stops`);
  lines.push('   when the CONDITION becomes FALSE.');
  traceOut('cf2-alarm-out', lines);
}
...
// 4. Packing an Egg Carton — FOR
function runCF2Eggs() {
  var slots = Math.max(1, Math.min(20, +document.getElementById('cf2-slots').value || 6));
  var lines = ['# Packing an Egg Carton — FOR Loop', ''];
  lines.push(`FOR slot FROM 1 TO ${slots}:`, '');
  for (var slot = 1; slot <= slots; slot++) {
    lines.push(`  slot = ${slot}  →  PickEggFromBasket 🥚 → PlaceEggInCarton(${slot}) ✅`);
  }
  lines.push('', `slot = ${slots+1} > ${slots}  →  STOP. Loop ends.`);
  lines.push('', `⬆ Total: ${slots} eggs packed — exactly the number of slots!`);
  traceOut('cf2-eggs-out', lines);
}
